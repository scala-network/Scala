...
process_distribution(bool cumulative, std::uint64_t start_height,
...
boost::optional<output_distribution_data> RpcHandler::get_output_distribution(
  ...
  static struct D {
    ...
    bool cached;
    D()
        : cached_from(0), cached_to(0), cached_start_height(0), cached_base(0),
          cached_m10_hash(crypto::null_hash),
          cached_top_hash(crypto::null_hash), cached(false) {}
  } d;
  ...
  if (d.cached && amount == 0 && d.cached_from == from_height &&
      d.cached_to == to_height && d.cached_top_hash == top_hash)
    return process_distribution(cumulative, d.cached_start_height,
                                d.cached_distribution, d.cached_base);
  ...
  if (!can_extend) {
    // we kept track of the hash 10 blocks below, if it exists, so if it
    // matches, we can still pop the last 10 cached slots and try again
    if (d.cached && amount == 0 && d.cached_from == from_height &&
      ...
      if (hash10 == d.cached_m10_hash) {
        ...
        CHECK_AND_ASSERT_MES(
            d.cached_distribution.size() >= 10, boost::none,
  ...
  if (can_extend) {
    ...
    if (!f(amount, d.cached_to + 1, to_height, start_height, new_distribution,
           base))
      return boost::none;
  ...
  if (amount == 0) {
    ...
    d.cached_m10_hash =
        d.cached_to >= 10 ? get_hash(d.cached_to - 10) : crypto::null_hash;
  ...
  return process_distribution(cumulative, start_height, std::move(distribution),
                              base);
}
} // namespace rpc
} // namespace cryptonote
